function jointProbabiliteGamma=jointProbabilite_QPSK_Gamma(bit_Alice,bit_Bob,Attenuation,ScaleCo,P_T_dBm,C2n) 
    ...
    t=[0.137793470540,0.729454549503,1.808342901740,3.401433697855,...
       5.552496140064,8.330152746764,11.843785837900,16.279257831378,21.996585811981,...
       29.920697012274];
    %vl-weight factor of Laguerre polynomial
    v=[0.308441115765,0.401119929155,0.218068287612,0.0620874560987,...
       0.00950151697518,0.000753008388588,0.0000282592334960,0.000000424931398496,0.00000000183956482398,...
    ...
    G_Tx_dB=110;                   %Tx telescope gain(dB)
    G_Tx=10^(G_Tx_dB/10);
    G_Rx_dB=120;                   %Rx telescope gain(dB)
    G_Rx=10^(G_Rx_dB/10);
    ...
    %Weather conditions
    %Parameters
    V_fog=50/1000;           %Fog visibility (km)
    sigma=0.585*V_fog^(1/3); 
    lamda_wavelength_m=1550; %Wavelength (nm) 
    delta_d_fog=50/1000;     %(km)
    R_rain=50;               %Rainfall rate (mm/hour)
    delta_d_rain=1000/1000;  %Rain layer thickness (km)
     
    %Thick Fog
    L_fog=4.343*(3.91./V_fog.*(lamda_wavelength_m./550).^(-sigma)).*delta_d_fog./cos(zenithAng);
    
    %Heavy Rain
    L_rain=1.076.*R_rain.^0.67.*delta_d_rain./cos(zenithAng);
    ...
    EI0=M*R*sqrt(G_Tx*1/FSL*P_T*hl*G_Rx*P_LO);
    EI1=-EI0;
    ...
    for i=1:length(t)
        a(i)=teta(i)/temp1;     
        for l=1:length(t)
            I0=M*R*sqrt(G_Tx*1/FSL*P_T*hl*t(l)/special(i)*G_Rx*P_LO);
            I1=-I0;
