function jointProbabiliteGamma=jointProbabilite_QPSK_Gamma(bit_Alice,bit_Bob,Attenuation,ScaleCo,P_T_dBm,C2n) 
    ...
    t=[0.137793470540,0.729454549503,1.808342901740,3.401433697855,...
       5.552496140064,8.330152746764,11.843785837900,16.279257831378,21.996585811981,29.920697012274];
    ...
    G_Tx=10^(G_Tx_dB/10);
    G_Rx_dB=110;                   %Rx telescope gain(dB)
    G_Rx=10^(G_Rx_dB/10);
    ...
    for i=1:length(t)
        ...
        for l=1:length(t)
            ...
            sigmaN_i_l=sqrt(2*q*M^(2+x)*(R*P_LO+R*G_Tx*1/FSL*P_T*hl*t(l)/special(i)*G_Rx+Id)*deltaf+4*kB*T/RL*deltaf);
